 ...
 *
 * Fixes applied:
 *   FIX 1: sigchld_handler now uses self-pipe (async-signal-safe). Main loop
 *          drains the pipe and calls the real reap logic in process context,
 *          where pthread_mutex_lock() is safe.
 *   FIX 2: child_fn receives a private copy of child_config on its own stack
 *          so the parent can return immediately without a use-after-free race.
 *   FIX 3: SIGPIPE ignored in supervisor init so writes to disconnected
 *          run-client FDs do not kill the supervisor.
 *   FIX 4: Producer threads are joinable. Their pthread_t is stored in
 *          container_record_t and joined during shutdown / container cleanup.
 *   FIX 5: run_fd is closed on all error paths in CMD_RUN handling.
 */
...
#define MAX_PRODUCERS       64   /* FIX 4: max tracked producer threads */
...
typedef struct container_record {
    ...
    int               log_pipe_read;
    int               run_client_fd;
    pthread_t         producer_tid;      /* FIX 4: joinable producer thread */
    int               producer_running;  /* FIX 4: 1 if tid is valid */
...
/*
 * FIX 2: child_config lives entirely on the child's clone stack.
 * The parent never needs to free it; the child owns it.
 * We embed it at the bottom of the stack allocation and pass a pointer
 * into the already-allocated region — no heap allocation needed.
 */
typedef struct {
    char id[CONTAINER_ID_LEN];
    char rootfs[PATH_MAX];
...
static void *logging_thread(void *arg) {
    ...
    while(1){
        ...
        char log_path[PATH_MAX]; log_path[0]='\0';
        pthread_mutex_lock(&ctx->metadata_lock);
        container_record_t *c=ctx->containers;
        while(c){if(!strncmp(c->id,item.container_id,CONTAINER_ID_LEN)){strncpy(log_path,c->log_path,PATH_MAX-1);log_path[PATH_MAX-1]='\0';break;}c=c->next;}
...
/* FIX 4: producer is joinable — no pthread_detach */
...
static int register_with_monitor(int fd,const char *id,pid_t pid,unsigned long soft,unsigned long hard){
...
static int unregister_from_monitor(int fd,const char *id,pid_t pid){
    struct monitor_request req; if(fd<0)return 0;
    memset(&req,0,sizeof(req)); req.pid=pid;
    strncpy(req.container_id,id,sizeof(req.container_id)-1);
    req.container_id[sizeof(req.container_id)-1]='\0';
    if(ioctl(fd,MONITOR_UNREGISTER,&req)<0)return -1;
    return 0;
}
...
/* ----------
 * FIX 1: real reap logic — called from process context (main loop),
 * NOT from the signal handler. Safe to use pthread_mutex_lock here.
 * ---------- */
        ...
        while(c){
            if(c->host_pid==pid){
                ...
                if(c->run_client_fd>=0){
                    ...
                    ssize_t wr=write(c->run_client_fd,&final,sizeof(final));
                    (void)wr;
...
/* FIX 1: signal handler only writes one byte to self-pipe — async-signal-safe */
static void sigchld_handler(int sig){
    (void)sig;
    if(g_ctx && g_ctx->sigchld_pipe[1]>=0){
        char b=1;
        /* write() is async-signal-safe */
        ssize_t r=write(g_ctx->sigchld_pipe[1],&b,1);
        (void)r;
    }
}
...
/* ----------
 * FIX 2: child_fn takes a pointer to child_config_t embedded at the bottom
 * of its own stack. The parent filled it in before clone(); the child reads it
 * before execv(). No shared heap pointer — no use-after-free race.
 * ---------- */
static int child_fn(void *arg){
    ...
    if(cfg->nice_value!=0){
        if(nice(cfg->nice_value)==-1&&errno!=0) perror("nice (non-fatal)");
    }
...
static int launch_container(supervisor_ctx_t *ctx,const control_request_t *req,control_response_t *resp,int run_client_fd){
    ...
    if(find_container(ctx,req->container_id)){
        pthread_mutex_unlock(&ctx->metadata_lock);
        resp->status=-1;
        snprintf(resp->message,CONTROL_MESSAGE_LEN,"container '%s' already exists",req->container_id);
        return -1;
    }
    pthread_mutex_unlock(&ctx->metadata_lock);
    ...
    /*
     * FIX 2: allocate stack + embedded config in one block.
     * config sits at the LOW address (bottom of allocation);
     * stack grows down from the HIGH end.
     * Layout: [child_config_t | padding ... | stack top]
     */
    size_t alloc_size = STACK_SIZE + sizeof(child_config_t);
    char *stack_mem = malloc(alloc_size);
    if(!stack_mem){
    ...
    /* cfg lives at the start of the allocation — child reads before execv */
    child_config_t *cfg = (child_config_t*)stack_mem;
    ...
    /* stack top is the high end of the allocation */
    char *stack_top = stack_mem + alloc_size;
    ...
    /*
     * FIX 2: stack_mem must NOT be freed here — it holds cfg which the child
     * may still be reading. We free it after the child exits via the record.
     * Store the pointer in the record so cleanup can free it.
     *
     * NOTE: In practice execv() replaces the child's address space so by the
     * time clone() returns in the parent the child has either already called
     * execv() (and no longer uses cfg) or is very shortly about to. A small
     * sleep(0) / sched_yield() here would be sufficient, but to be strictly
     * correct we keep the pointer until container teardown.
     */
    ...
    /* FIX 4: create joinable producer thread, store tid in record */
    ...
    if(pa){
        ...
        if(pthread_create(&rec->producer_tid,NULL,producer_thread,pa)==0){
            ...
            rec->log_pipe_read=-1; /* producer owns the fd now */
        } else {
            free(pa);
            close(pipefd[0]);
            rec->log_pipe_read=-1;
    ...
    /* FIX 2: free stack_mem now — by the time clone() returned in the parent
     * the child has already copied what it needs (it's in its own address space
     * after CLONE_NEWPID|CLONE_NEWNS; it sees a private copy of the mapping).
     * Actually with clone() without CLONE_VM the child has a full copy-on-write
     * fork of the address space, so cfg is valid in the child independently.
     * Safe to free in parent now. */
    free(stack_mem);
...
static void handle_request(supervisor_ctx_t *ctx,int client_fd,const control_request_t *req){
    ...
    case CMD_START:
        ...
        break;
    case CMD_RUN:{
        /* FIX 5: dup run_fd, close on every error path */
        int run_fd=dup(client_fd);
        if(run_fd<0){
            resp.status=-1;
            snprintf(resp.message,CONTROL_MESSAGE_LEN,"dup failed");
            (void)write(client_fd,&resp,sizeof(resp));
            break;
        }
        int rc=launch_container(ctx,req,&resp,run_fd);
        if(rc!=0){
            /* FIX 5: launch failed — run_fd was not stored, close it here */
            close(run_fd);
        }
        /* always send initial ack (success or failure) */
        (void)write(client_fd,&resp,sizeof(resp));
        break;
    }
    case CMD_PS:{
        char buf[4096]; int off=0; buf[0]='\0';
        ...
        while(c&&off<(int)sizeof(buf)-80){
            char tstr[32]; struct tm *tm_p=localtime(&c->started_at);
            strftime(tstr,sizeof(tstr),"%H:%M:%S",tm_p);
            off+=snprintf(buf+off,(int)sizeof(buf)-off,
                          "%-16s %-8d %-10s %-10s %-10lu %-10lu\n",
                          c->id,c->host_pid,state_to_string(c->state),tstr,
        ...
        resp.message[CONTROL_MESSAGE_LEN-1]='\0';
        (void)write(client_fd,&resp,sizeof(resp));
        break;
    }
    case CMD_LOGS:{
        ...
        if(c){ strncpy(log_path,c->log_path,PATH_MAX-1); log_path[PATH_MAX-1]='\0'; }
        pthread_mutex_unlock(&ctx->metadata_lock);
        if(!log_path[0]){
            resp.status=-1;
        ...
        if(fd<0){
            resp.status=-1;
            snprintf(resp.message,CONTROL_MESSAGE_LEN,"log not found: %s",log_path);
            (void)write(client_fd,&resp,sizeof(resp));
            break;
        }
        resp.status=0;
        strncpy(resp.message,"OK",CONTROL_MESSAGE_LEN-1);
        (void)write(client_fd,&resp,sizeof(resp));
...
static int run_supervisor(const char *rootfs){
    ...
    ctx.sigchld_pipe[0]=-1; ctx.sigchld_pipe[1]=-1;
    g_ctx=&ctx;
    ...
    if(ctx.monitor_fd<0)
        fprintf(stderr,"[supervisor] kernel monitor not available (load monitor.ko for memory limits)\n");
    ...
    /* FIX 1: self-pipe for SIGCHLD — both ends non-blocking */
    if(pipe(ctx.sigchld_pipe)!=0){perror("pipe sigchld");goto cleanup;}
    fcntl(ctx.sigchld_pipe[0],F_SETFL,O_NONBLOCK);
    fcntl(ctx.sigchld_pipe[1],F_SETFL,O_NONBLOCK);

    /* FIX 3: ignore SIGPIPE so writes to closed run-client FDs don't kill us */
    ...
    while(!ctx.should_stop){
        ...
        FD_SET(ctx.sigchld_pipe[0],&rfds);
        int maxfd = ctx.server_fd > ctx.sigchld_pipe[0]
                    ? ctx.server_fd : ctx.sigchld_pipe[0];
        struct timeval tv={.tv_sec=1,.tv_usec=0};
        int nready=select(maxfd+1,&rfds,NULL,NULL,&tv);
        if(nready<0){if(errno==EINTR)continue;perror("select");break;}
        if(nready==0)continue;

        /* FIX 1: drain self-pipe, then reap in process context */
        if(FD_ISSET(ctx.sigchld_pipe[0],&rfds)){
            char drain[64];
            while(read(ctx.sigchld_pipe[0],drain,sizeof(drain))>0);
            reap_children(&ctx);
        }
...
cleanup:
    /* FIX 4: join all producer threads before shutting down the log buffer */
    pthread_mutex_lock(&ctx.metadata_lock);
    c=ctx.containers;
    while(c){
    ...
    while(cur){
        ...
        if(cur->log_pipe_read>=0) close(cur->log_pipe_read);
        if(cur->run_client_fd>=0) close(cur->run_client_fd);
        free(cur);
        cur=nxt;
    }
    ctx.containers=NULL;
    ...
    if(ctx.monitor_fd>=0)close(ctx.monitor_fd);
    if(ctx.sigchld_pipe[0]>=0)close(ctx.sigchld_pipe[0]);
    if(ctx.sigchld_pipe[1]>=0)close(ctx.sigchld_pipe[1]);
    pthread_mutex_destroy(&ctx.metadata_lock);
    g_ctx=NULL;
...
static void forward_stop_to_supervisor(const char *container_id){
    ...
    ssize_t wr=write(fd,&req,sizeof(req)); (void)wr;
    control_response_t resp;
    ssize_t rd=read(fd,&resp,sizeof(resp)); (void)rd;
    close(fd);
}
...
static int send_control_request(const control_request_t *req){
    ...
    if(req->kind==CMD_RUN){
        ...
        g_run_container_id[CONTAINER_ID_LEN-1]='\0';
        struct sigaction sa; memset(&sa,0,sizeof(sa)); sigemptyset(&sa.sa_mask);
